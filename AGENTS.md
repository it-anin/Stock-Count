...
6. **ถ้ากำลังไล่หาสาเหตุว่า "ข้อมูลบน Firestore หายไปไหน" ให้อ่าน `CLAUDE.md` §กฎ 0 ก่อนตั้งสมมติฐานใดๆ**
   ลำดับที่ถูกคือ **(1) ใครกดอะไรไปล่าสุด — ดู `countResetAt` ก่อน → (2) log ของตัวเขียนบนเครื่องที่รันจริง → (3) ค่อยสงสัยว่าโค้ดพัง**
   ⚠️ **ระบบนี้มีหลายเครื่อง** — auto-r01 รันบน `BIGYAMAINPC` ไม่ใช่เครื่องพัฒนา · เช็ค Task Scheduler ผิดเครื่องแล้วสรุปว่า "บอทไม่ได้รัน" เคยเกิดมาแล้ว (ก.ย. 2026)
...
- **`auto-r05/` ใช้งานจริงแล้ว (9 ก.ย. 2026)** — Task `AutoR05Import` 09:00 บน `BIGYAMAINPC` อัป R05.106 ลง `global_r05` ทุกเช้า
  - เขียนครั้งแรกสำเร็จ 11:33 น. `global_r05` 10,857 → **10,864 บาร์โค้ด** (7 ตัวที่ตกหล่นมาตั้งแต่ 2 ก.ย. ยิงไม่ติดบน PDA อยู่ทั้งสัปดาห์)
  - ✅ **ด่านกันเขียนซ้ำพิสูจน์บนของจริงแล้ว** — รันซ้ำได้ "เนื้อหาเหมือนเดิมทุกไบต์ ไม่เขียน" ⇒ Firestore เก็บสตริงกลับมาเหมือนเดิม วันที่บาร์โค้ดไม่เปลี่ยนจึงไม่มี write ไม่มี toast กวนใคร
  ...
  - **สัญญาณ "บอทตรวจแล้ววันนี้" อยู่ที่ doc แยก `stock_sessions/global_r05_status` (`checked_at`) ไม่ใช่ใน `global_r05`** (ก.ย. 2026) — บอทเขียนทุกรอบที่ตรวจสำเร็จ รวมรอบ "เนื้อหาเหมือนเดิม" แต่ **ห้ามเขียนเมื่อตกด่าน** · การ์ด R05.106 โชว์คู่กับ "อัปโหลดล่าสุด" และเตือนเมื่อไม่ใช่วันนี้
...
- **ข้อยกเว้นที่ตั้งใจของกฎก่อนหน้า: ใบปรับปรุงคำนวณจาก `si.systemQty` ค่าสด** (`_buildAdjustDocRows()`) เพราะเป้าหมายคือ *ทำให้ระบบเหลือเท่ากับยอดที่นับได้* — เลขที่ส่งไปจะถูกบวก/ลบกับยอดที่ระบบมี ณ ตอนนั้น ⛔ **ห้ามเปลี่ยนไปคำนวณด้วย `recheckSystemQty`**
  แลกด้วยเงื่อนไข: **ยอดรีเช็คต้องยังสด** — `_isAdjustRowFresh()` บังคับ `_recheckBaselineSystemQty()===live` ถึงจะขึ้นใบ · ไม่สด = ตัดออก ต้องรีเช็คใหม่ (ก.ย. 2026 · เดิม R01 ขยับจน `diff` เป็น `0` แล้วแถวหลุดทั้ง ORDS+IRPS เงียบสนิท ขณะที่ badge ยังนับ)
  ⚠️ **ตัวกรองต้องอยู่ใน `_buildAdjustDocRows()` จุดเดียว** (ตาราง/Export Text/Export Excel ใช้ประตูเดียวกัน) · `_adjustDocAudit()` คืน `{stale,noSku,settled}` ให้ toast ตอน Export (`_warnAdjustDocDropped()`) รายงาน — เพิ่มเงื่อนไข `continue` ใหม่เมื่อไร ต้องนับเข้าตัวรายงานเสมอ · แถบ `#adjustDocWarn` ในป็อปอัพถูกถอดตามที่ผู้ใช้สั่ง (24 ก.ย. 2026) ⇒ toast นี้เป็นสัญญาณเดียวที่เหลือ ห้ามถอดตาม · `exportStockAdjExcel()` จงใจไม่ใช้ด่านนี้ (เป็นรายงาน ไม่ใช่ไฟล์ import ERP)
  ⚠️ **ทั้งข้อนี้ใช้กับสาขายาเท่านั้น — WH คิด Stock Adj จาก "R01 ของวันที่ Recheck" (24 ก.ย. 2026 · ผู้ใช้สั่ง)**: ฐาน = `sd.systemQty` ที่แช่ไว้ตอนยืนยันรีเช็ค ผ่าน `_whStockAdjBaseQty()` ครบ 4 จุด (ใบ ORDS/IRPS · Audit Verify แท็บ Stock Adj · ประวัติการนับ · `exportStockAdjExcel`) และไม่มีด่านความสด · ผลข้างเคียง: ใบ WH ไม่หายเองหลังส่ง ERP ⇒ Export ซ้ำได้เลขเดิม ระวังส่งซ้ำ · เทส `wh-stock-adj-base.spec.js`
...
`_applyScanItemRemoved`, `_applyCloudSessionMeta`, `_loadScanItemsFromCloud`, `_deleteScanItemsForEpoch`, `_deleteScanItemsNotInEpoch`, `_restampBranchConfirmLock`,
...
| (ก.ย. 2026) | **หมวด `DELETE` ถูกตัดออกจากงานนับทั้งหมด ทั้งที่ 776 รายการยังมียอดคงเหลือ** (SRC 327 · KKL 220 · SSS 224 · WH 5 · ในนั้นติดลบ 29 = ค้างส่งลูกค้า) และทุกตัว**มีบาร์โค้ดใน R05 ครบ** ⇒ ของจริงบนชั้นไม่เคยถูกส่งไปนับ | ถอด `DELETE` ออกจาก `R01_NON_COUNT_KEYWORDS` (เหลือ prefix `11.` อย่างเดียว) — ⛔ **ห้ามเติมกลับ** · ตัวที่ยอดเป็น 0 ถูกกติกา `G ≠ 0` ตัดออกเองอยู่แล้ว · **ห้ามลบตัวแปร** ต้องคงเป็น array ว่าง (`tools/check-r01-parity.js` ดึงด้วย regex) · กติกาอยู่ **3 ไฟล์** (`index.html` · `auto_r01_import.py` · `tools/list-r01-categories.js`) แก้พร้อมกันแล้วรัน parity checker เสมอ · **บทเรียน: ธงที่ถูกตรึงตอน parse ต้องมีเครื่องมือ resync บน cloud เสมอ** ไม่งั้นแก้กติกาแล้วมีผลช้าไปหนึ่งวัน → `--resync-nc` เขียนเฉพาะ `data_json` (ไม่แตะ version/baseline/r16 จึงไม่ล้าง R16 ไม่ freeze audit) และต้องรัน**ก่อน** deploy |
| (ก.ย. 2026 รอบ 2) | ถอด `DELETE` ออกจากหมวดที่ตัดทิ้งแล้ว แต่สูตรใช้ **หรือ** ระหว่าง "ยอดไม่เป็น 0" กับ "ชั้น A/B/C/REVIEW" ⇒ ของหมวด DELETE ที่ยังอยู่ใน PBM ด้วยชั้น A/B/C/REVIEW และยอด 0 จะถูกดึงเข้า Progress ทั้งที่ถูกมาร์คลบไปแล้ว | **ธง `nc` แยก 2 ชนิด**: `nc:1` (หมวด `11.`) ตัดเด็ดขาด · `nc:2` (หมวด `DELETE`) ตัดสินด้วยยอดอย่างเดียว ชั้น A/B/C/REVIEW ดึงเข้าไม่ได้ · เก็บเป็นตัวเลข **ห้ามเก็บข้อความหมวด** (เพดาน 1 MiB) · อ่านแบบ truthy + เก็บค่าเข้มสุด (`1` ชนะ `2`) เพื่อให้ doc เก่าที่มีแต่ `nc:1` ให้ผลเท่าเดิม · เพิ่มชนิดใหม่ต้องเติม regex ใน `tools/check-r01-parity.js` ด้วย ไม่งั้น parity ตรวจไม่ครบ |
...
| (ก.ย. 2026 · SRC 26 ก.ย.) | **กดเริ่มนับใหม่ (22 ก.ย.) แล้ว Pass ของรอบ 19 ก.ค. โผล่กลับมาในรอบใหม่ทีละตัวตามจังหวะที่สแกน** — (1) ขั้นลบ items รอบก่อนใน `startNewCount` ถูกตัดกลางคัน (login ใหม่ 8 วินาทีหลังกด) เศษรอบเก่า ≥500 docs ค้าง **ใต้ doc id เดียวกับ SKU** (2) `_writeScanningItem` เปิดเจอ pass ของรอบเก่าแล้วถือว่า "เครื่องอื่น Confirm แล้ว" → เอามาแทนยอดที่เพิ่งสแกน · SKU ยัง dirty จากการยิงซ้ำระหว่างรอ → plain flush เขียนผลรอบเก่ากลับในนามรอบใหม่ (ลายเซ็น rev 2) (3) schema v2 ไม่มีด่าน "confirmed ในเครื่องแต่ Cloud ไม่มี ห้ามดันกลับ" → login ใหม่แล้ว reconcile ดัน localStorage ขึ้นไป (ลายเซ็น rev 1 · batch เดียว) | **doc id = SKU ใช้ร่วมทุกรอบ ⇒ ทุกจุดที่อ่าน item "ตาม id" ต้องเช็ค `countResetAt` เอง** — เศษรอบเก่าไม่ได้ไร้พิษอย่างที่ comment ของ `_deleteScanItemsForEpoch` เชื่อ (query ตามรอบกรองได้ แต่ `tx.get(ref)` ไม่ได้) · `_writeScanningItem`: เอกสารรอบอื่น = "ไม่มี" · ⛔ ห้าม abort `confirmed` จากเอกสารที่ไม่ใช่รอบนี้ · login (สาขายา): confirmed ในเครื่องที่ Cloud รอบนี้ไม่มีเอกสาร → รีเซ็ตเป็น pending ก่อน reconcile (marker รอบนี้สร้างกลับเองได้) · เทส `tests/specs/e2e/stale-round-resurrect.spec.js` (พิสูจน์แล้วว่าโค้ดก่อนแก้ล้มแบบเดียวกับ production) · ล้างเศษบน cloud: `tools/cleanup-stale-round-items.js` · ไล่อาการ: `tools/diagnose-reset-resurrect.js` · ✅ (26 ก.ย. 2026 ต่อ) `startNewCount` ลบทุกรอบที่ไม่ใช่รอบใหม่ (`_deleteScanItemsNotInEpoch`) · ถือ lock ทุกสาขาที่ใช้ lock (เดิมเฉพาะ WH) แล้ว `_restampBranchConfirmLock` ด้วยรอบใหม่ก่อนลบ · แถบบังจอ + `beforeunload` + heartbeat ไม่รีโหลด · ลบไม่ครบต้อง toast · ✕ ห้ามลบรายการที่ Confirm แล้ว · abort `confirmed` ต้องอัปเดตแถว RESULT (เดิมแถวค้าง scanning + ✕ → ลบผลยืนยันทิ้ง) · เทสครบใน `stale-round-resurrect.spec.js` (พิสูจน์แล้วว่าโค้ดก่อนแก้ล้ม) · ⛔ ห้ามกดเริ่มนับใหม่เพื่อแก้อาการนี้ — ล้างยอดที่พนักงานสแกนค้างทั้งสาขา ใช้ cleanup script แทน |
...
- **ทั้ง 4 สาขาเป็น v2 แล้ว (7 ก.ย. 2026)** — SRC/WH ผ่าน `migrateSessionToSchemaV2()` ระหว่างรอบนับ (24 ก.ค.) · SSS (5 ส.ค.) และ KKL (7 ก.ย.) cutover เองตอน `startNewCount()`
  ⚠️ `startNewCount()` **ไม่สำรอง blob ให้** (ต่างจาก migrate) ⇒ KKL/SSS ไม่มี `{branch}_v1_backup` เลย · ต้อง `tools/backup-branch.js` ก่อนกดเสมอ
  ⚠️ `startNewCount()` **ลบ `{branch}_r01` ด้วย** ⇒ กดช่วงบ่าย/เย็นให้ auto-r01 เติมให้เช้าวันรุ่งขึ้น ถ้ากดหลังบอทรันจะเสียไปหนึ่งวัน
