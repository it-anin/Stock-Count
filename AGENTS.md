...
- **ข้อยกเว้นที่ตั้งใจของกฎก่อนหน้า: ใบปรับปรุงคำนวณจาก `si.systemQty` ค่าสด** (`_buildAdjustDocRows()`) เพราะเป้าหมายคือ *ทำให้ระบบเหลือเท่ากับยอดที่นับได้* — เลขที่ส่งไปจะถูกบวก/ลบกับยอดที่ระบบมี ณ ตอนนั้น ⛔ **ห้ามเปลี่ยนไปคำนวณด้วย `recheckSystemQty`**
  ...
  ⚠️ **ตัวกรองต้องอยู่ใน `_buildAdjustDocRows()` จุดเดียว** (ตาราง/Export Text/Export Excel ใช้ประตูเดียวกัน) · `_adjustDocAudit()` คืน `{stale,noSku,settled}` ให้ `_renderAdjustDocWarn()` รายงาน — เพิ่มเงื่อนไข `continue` ใหม่เมื่อไร ต้องนับเข้าตัวรายงานเสมอ · `exportStockAdjExcel()` จงใจไม่ใช้ด่านนี้ (เป็นรายงาน ไม่ใช่ไฟล์ import ERP)
