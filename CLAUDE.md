...
| auto-adj (LOT/ราคา ใบปรับปรุง → Supabase) | **✅ อัปข้อมูลจริงขึ้น Supabase สำเร็จครั้งแรก 12 ก.ย. 2026** (รันด้วยมือจากเครื่อง `BIG-IT`) — ตาราง+RLS สร้างแล้ว · `adj_r14_lots` = 35,311 คู่ SKU+LOT · 5,140 SKU (จาก `ADJ_R14.102.CSV` 130,408 แถว) · `adj_r05_prices` = 6,674 SKU มีราคาครบ (จาก `ADJ_R05105.CSV` 38,878 แถว) · รันซ้ำได้ "เนื้อหาเหมือนเดิม ไม่เขียน อัปเดตแค่ `checked_at`" พิสูจน์บนของจริงแล้ว · หัวคอลัมน์ R14.102 ยืนยันแล้ว (คอลัมน์ที่ 1 ชื่อ `EXPIREDATE` ไม่ใช่ `CF_EXPIREDATE_TEXT` ที่อยู่คอลัมน์ที่ 2 — `index.html` อ่านคอลัมน์ที่ 1 มาตลอด **ห้ามย้ายข้างเดียว**) · ⏳ **เหลือ:** ① `ADJ_R05105.CSV` ยังไม่ export ทุกวัน (ล่าสุด 7 ก.ย.) ② ยังไม่ลงทะเบียน Task `AutoAdjImport` (ต้องลงบนเครื่องที่ได้ไฟล์ทั้งสองทุกเช้า · `BIG-IT` มี R14 สดทุกวันจาก `BOTR14102`) ③ ยังไม่ deploy เว็บ ⇒ ป็อปอัพยังใช้แนบไฟล์/💾 แบบเดิม · ⛔ **ไฟล์ราคาต้องชื่อ `ADJ_R05105.CSV` (มีคำนำหน้า) ห้ามใช้ `R05.105.CSV` ล้วน** — `auto-r05` (**ห้ามแก้ ทำงานดีอยู่แล้ว**) ค้นด้วย `R05*.CSV` ซึ่งกินชื่อล้วนไปด้วย · ห้ามต่อวันที่ท้ายชื่อ (วันที่บางวันมีเลข 105/106 ปน) | `adj_meta.checked_at` ต้องเป็นวันนี้ · การ์ดในป็อปอัพไม่มี `⚠️ บอทยังไม่ได้ตรวจวันนี้` |
