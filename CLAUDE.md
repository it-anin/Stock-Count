...
- หมวดที่ไม่นับ: คอลัมน์ P ขึ้นต้น `11.` เท่านั้น — `_isNonCountR01Category()` ติดธง `nc:1` ตอน `loadR01` (เพิ่มหมวดใหม่แก้ที่ `R01_NON_COUNT_PREFIXES`/`R01_NON_COUNT_KEYWORDS`) · **หมวด R01 ชนะทั้งสองข้อเสมอ**
- ⛔ **ถอด `DELETE` ออกจาก `R01_NON_COUNT_KEYWORDS` แล้ว (ก.ย. 2026) ห้ามเติมกลับ** — ของจริง **776 รายการหมวด DELETE ยังมียอดคงเหลือ** (SRC 327 · KKL 220 · SSS 224 · WH 5 · ในนั้นติดลบ 29) และ**มีบาร์โค้ดใน R05 ครบทุกตัว** = ของบนชั้นจริงที่ต้องเดินไปนับ ไม่ใช่ของนอกคลัง · ตัวที่ยอดเป็น 0 ถูกกติกา `G ≠ 0` ตัดออกเองอยู่แล้ว จึงไม่ต้องพึ่ง keyword · `R01_NON_COUNT_KEYWORDS` ต้องคงอยู่เป็น array ว่าง **ห้ามลบตัวแปร** (`tools/check-r01-parity.js` ดึงบรรทัดนี้ด้วย regex)
- ⚠️ **กติกาตัดหมวดถูกทำซ้ำ 3 ที่ ต้องแก้พร้อมกันเสมอ:** `index.html` · `auto-r01/auto_r01_import.py` · `tools/list-r01-categories.js` แล้วรัน `node tools/check-r01-parity.js "<Allstock.CSV>"` ยืนยันว่าอัปผ่านเว็บกับให้บอทอัปได้ผลตรงกันทุกไบต์
