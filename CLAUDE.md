...
- R01.102 gate: **ปลดออกแล้ว** — `onclick` ของ upload item เปิด file picker ได้ทุกเวลาทุกสาขา (เดิมกั้น `getHours() < 21` เฉพาะสาขา; ถอดออกให้สาขาอัพ R01 ได้ยืดหยุ่นโดยไม่ต้องใช้ admin). guide text เปลี่ยนเป็น "แนะนำหลัง 21:00 · อัพได้ทุกเวลา". ⚠️ ยังควรอัพ R01 **ครั้งเดียวตอนเริ่มรอบ** — อัพซ้ำกลางรอบ = baseline เลื่อน (ดู R01 cross-day)
...
- **R01.102**: Col E (index 4)=SKU, F (5)=ProductName, G (6)=SystemQty; ข้ามเฉพาะแถวที่ qty **ว่าง/ไม่ใช่ตัวเลข** (junk). **qty≤0 (ติดลบ/0) ถูกเก็บไว้ด้วยค่าจริง** (`systemQty:+g` ไม่ clamp) — เพื่อให้สินค้า **DEL ที่ระบบติดลบ/0 แต่มีของจริงบนชั้น ถูกนับ** (เดิมตัด qty≤0 ทิ้ง → DEL พวกนี้หายไป เพราะ DEL = R01-ไม่อยู่-PM มาจาก r01Data เท่านั้น; PM items อยู่ skuMap จาก PM list อยู่แล้ว). `systemQty` อาจติดลบได้ → eval เทียบกับค่าติดลบ (ของจริง > ค่าติดลบ = Audit, Diff สะท้อนการปรับสต็อกจริง). Re-uploading clears previous data (`state.r01Data = []` first) then calls `rebuildMaps()`. ⚠️ **scanData ของ SKU เดิมไม่ถูกรีเซ็ตเป็น pending** — `rebuildMaps()` เพิ่ม pending เฉพาะ SKU ใหม่ที่ยังไม่มีใน scanData (`if(!state.scanData.has(sku))`) เท่านั้น. ของค้างถูกล้างด้วย `startNewCount()` หรือ `resetStaleScanningItems()` (login) เท่านั้น
  - ⚠️ **Re-upload เปลี่ยน `systemQty` (baseline การเทียบ):** `rebuildMaps()` สร้าง `skuMap` ใหม่จาก r01Data ล่าสุด → `systemQty` ของทุก SKU กลายเป็นค่าของไฟล์ที่เพิ่งอัพ. การรีเช็ค (`confirmAuditVerifyItem` เทียบ `pharmacistQty === si.systemQty`) และ Confirm (`evaluatePendingScans` เทียบ `effectiveCnt === sys`) อ้างอิง `systemQty` ปัจจุบันเสมอ → **อย่า re-upload R01 ในวันรีเช็ค (cross-day)** เพราะ baseline จะเลื่อนไปเป็นเลขวันใหม่ (สต็อกอาจเปลี่ยนข้ามคืน) → pass/stock_adjustment เพี้ยน. R01 ของวันก่อน persist ผ่าน Firestore master doc (`${branch}_r01`) → `restoreMasterFromFirestore()` โหลดกลับตอนเปิดแอป จึงรีเช็คต่อได้เลยโดยไม่ต้องอัพใหม่. SKU ที่หายจากไฟล์ใหม่ → `skuMap` ไม่มี → audit item นั้นรีเช็คไม่ได้ (`si` undefined). (qty≤0 ไม่ทำให้หายแล้ว — เก็บไว้ด้วยค่าจริง)
