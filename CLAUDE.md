...
- **`applyAuditTerminology()`** → อัปเดต label ที่เป็น static HTML (มี id: `statAuditLabel`, `auditVerifyPanelTitle`, `auditVerifyBtnLabel`, `popupFilterAuditLabel`, `popupThAudit`, `avTabAuditLabel`) ตาม branch — เรียกใน `initAfterLogin()` + ตอนเลือกพนักงาน. การ์ด AUDIT label ยัง refresh ทุก `updateStats()` ด้วย
  ...
  - **`body.branch-wh` class** — `applyAuditTerminology()` ทำ `document.body.classList.toggle('branch-wh', currentBranch==='WH')` เป็น hook สำหรับ scope CSS เฉพาะ WH. ปัจจุบันใช้บน **WH PDA เท่านั้น** (กฎ `.branch-wh ...` ใน `@media(max-width:600px)`): (1) ลด font ตาราง RESULT ทั้งตาราง **0.50rem** (~8px); (2) ปรับ grid คอลัมน์เป็น `70px 1fr 40px 78px` (SKU **85→70**, QTY **52→40**, Status **60→78**) + ช่องกรอก `.inline-qty-input` **52→36px** + ลด padding `.scan-row-qty` เหลือ 2px (ให้ input 36px พอดี ไม่ถูก clip). QTY แคบลงเพิ่มพื้นที่ชื่อ (1fr); Status กว้างขึ้นให้ "⚠️ Recheck" แสดงเต็ม (เดิม 60px ตัดเหลือ "Rechec"); SKU ลดเพราะมี slack (SKU + ปุ่ม ✕ ยังพอ). สาขา PDA + ทุก Desktop ไม่กระทบ (ไม่มี class / media query ไม่เข้า)
