...
`_r01BaselineAt` (module-level `let`) — ISO timestamp ล่าสุดที่อัพ R01 บน **สาขายา (SRC/KKL/SSS)** persisted ใน localStorage + Firestore session (`r01BaselineAt` ใน `saveSession`/`syncToFirestore`). ใช้ detect อัพ R01 ข้ามเครื่อง — ถ้า `cloud.r01BaselineAt > local._r01BaselineAt` → `_applyR01BaselineUpdate()` โหลด R01 จาก `${branch}_r01` master doc + `resetUnverifiedAuditForNewR01()`. รีเซ็ตใน `startNewCount()`. Helper `_isPharmacyBranch()` → `currentBranch==='SRC'||'KKL'||'SSS'`
...
**📋 popup — staff filter row (ทุกสาขา + WH):** ป็อปอัพมี toolbar 2 แถว — แถวบน `#popupStatusFilterRow` (status filter เดิม: ทั้งหมด/Pass/Audit/Unknown/DEL/รอนับ, `popupFilterState`), แถวล่าง `#popupStaffFilterRow` (filter ตามชื่อผู้สแกน, `popupStaffFilter`, default `'all'`). `renderPopupStaffButtons()` (เรียกใน `openStockPopup`) สร้างปุ่ม "ทุกคน" + ชื่อพนักงานจาก `EMPLOYEE_PROFILES[currentBranch]` — **WH ใช้ `profiles.warehouse`** (พนักงานแพ็คกิ้ง: มุก/ตั๋ง/แล็ค/พี, ใช้บน WH supervisor Desktop), **สาขาใช้ `profiles.assistant`** (ไม่รวม pharmacist); ซ่อนถ้าไม่มีชื่อ. `renderPopupTable` กรอง 2 ชั้นแบบ AND: `popupFilterState` แล้วตามด้วย `(r.sd.scannedBy||'')===popupStaffFilter`. `setPopupFilter`/`setPopupStaffFilter` toggle `.active` แยกกันด้วย row id (กันชน เพราะปุ่มใช้ class `.popup-filter` เหมือนกัน). reset เป็น `'all'` ทุกครั้งที่เปิดป็อปอัพ
...
1. **Upload files** → `loadR01()` / `loadR05()` / `loadProductMaster()` → `rebuildMaps()` builds `skuMap`, `barcodeMap`, `skuDirectMap` and initialises `scanData` with `pending` entries for every known SKU.
   - **สาขายา — อัพ R01 ซ้ำ (`loadR01`):** หลัง `rebuildMaps()` ตั้ง `_r01BaselineAt` ใหม่ → `resetUnverifiedAuditForNewR01()` รีเซ็ต item ที่ `status==='audit' && !auditor` กลับ `pending` (ล้าง countedQty, ลบจาก scanListMap + `_avMap`) → `reEvaluateAuditItems({silent:true})` ปรับ `pass` ที่ยังไม่ยืนยันตาม baseline ใหม่. **WH ไม่รีเซ็ต.** รายการที่เภสัชยืนยันแล้ว (`auditor` มีค่า) ไม่ถูกแตะ. Cross-device: `_applyR01BaselineUpdate` ใน listener / `syncToFirestore` / `pullFromCloud` / `restoreFromFirestore`
...
6. **Audit resolution** → pharmacist opens Audit Verify panel → scans in dedicated input → clicks **ยืนยันทั้งหมด** → status changes to `pass` or `stock_adjustment`. เทียบ `effectiveQty = pharmacistQty + R16(ตามเวลาสแกนผู้ช่วย)` vs `systemQty` ใน `confirmAuditVerifyItem` / `getPharmacistAuditEffectiveQty`

**สาขายา — workflow รายวัน (หลัง restock):**

| วัน | ขั้นตอน |
|---|---|
| **Day 1** | อัพ R01 → ผู้ช่วยสแกน → อัพ R16 → Confirm → ติด Audit (ถ้าไม่ตรง) |
| **Day 2** | อัพ R01 (Audit ค้างรีเซ็ตเป็น pending) → **ผู้ช่วยสแกนใหม่** → อัพ R16 วันนั้น → Confirm → เภสัช Audit Verify |

⚠️ หลังอัพ R01 สถานะเป็น `pending` — **เภสัชสแกน Audit ไม่ได้ทันที** (`processPharmacistAuditScan` รับเฉพาะ `status==='audit'`). ถ้าต้องการให้เภสัชตรวจ Audit ของเมื่อวานโดยไม่นับใหม่ → **อย่าอัพ R01 ก่อนยืนยัน**

**R16 สาขายา — อัพรายวันได้** (ไม่ต้องสะสมหลายวันในไฟล์เดียว) ถ้าอัพ R01 ทุกวัน + สแกน/Confirm ในวันเดียวกัน — แต่ละครั้ง `loadR16` **แทนที่** maps ทั้งก้อน; TRANDATE filter ยังใช้เวลาสแกนของแต่ละ SKU. **ข้อควรระวัง:** `pass` ที่ยังไม่มี `auditor` และ `timestamp` วันก่อน + R16 วันนี้อย่างเดียว → Confirm อาจเพี้ยน
...
- **`applyAuditTerminology()`** → อัปเดต label ที่เป็น static HTML (มี id: `statAuditLabel`, `auditVerifyPanelTitle`, `auditVerifyBtnLabel`, `popupFilterAuditLabel`, `popupThAudit`, `avTabAuditLabel`) ตาม branch — เรียกใน `initAfterLogin()` + ตอนเลือกพนักงาน. การ์ด AUDIT label ยัง refresh ทุก `updateStats()` ด้วย
  ...
  - **`body.branch-wh` class** — `applyAuditTerminology()` ทำ `document.body.classList.toggle('branch-wh', currentBranch==='WH')` เป็น hook สำหรับ scope CSS เฉพาะ WH. ปัจจุบันใช้บน **WH PDA เท่านั้น** (กฎ `.branch-wh ...` ใน `@media(max-width:600px)`): (1) ลด font ตาราง RESULT ทั้งตาราง **0.50rem** (~8px); (2) ปรับ grid คอลัมน์เป็น `85px 1fr 40px 78px` (SKU **85 คงเดิม** — สำคัญต้องเต็ม, QTY **52→40**, Status **60→78**) + ช่องกรอก `.inline-qty-input` **52→36px** + ลด padding `.scan-row-qty` เหลือ 2px (ให้ input 36px พอดี ไม่ถูก clip). QTY แคบลง; Status กว้างขึ้นให้ "⚠️ Recheck" แสดงเต็ม (เดิม 60px ตัดเหลือ "Rechec"); ชื่อสินค้า (1fr) รับพื้นที่ที่เหลือ. **Trade-off:** บน PDA แคบ การให้ SKU+Status เต็มทั้งคู่ทำให้ชื่อได้พื้นที่ที่เหลือ (font 0.50rem ช่วยให้ยังโชว์ได้พอควร). สาขา PDA + ทุก Desktop ไม่กระทบ (ไม่มี class / media query ไม่เข้า)
...
|---|---|---|---|
| R01.102 upload | **ไม่มี gate (ปลดแล้ว)** | — | — |
| R16.104 upload | **ไม่มี gate (ปลดแล้ว)** | — | — |
| R16.103 upload | ไม่มี gate | — | WH only |
| Scan (all roles) | 08:00 – 20:59 | ✗ | ✗ |

- R01.102 gate: **ปลดออกแล้ว** — อัพได้ทุกเวลาทุกสาขา. **สาขายา (SRC/KKL/SSS):** ออกแบบให้อัพ **ทุกวันก่อน Confirm** (= baseline หลัง restock) — Audit ที่ยังไม่ยืนยันจะรีเซ็ต (ดู `resetUnverifiedAuditForNewR01`). **WH:** อัพครั้งเดียวตอนเริ่มรอบ — อัพซ้ำกลางรอบ baseline เลื่อน → recheck/Confirm เพี้ยน
- R16.104 gate: **ปลดออกแล้ว** — อัพได้ทุกเวลา. **สาขายา:** อัพ **รายวัน** ได้ (ช่วงหลังอัพ R01 ถึง Confirm — ไม่จำเป็นต้องสะสมหลายวันในไฟล์เดียวถ้าอัพ R01 ทุกวัน). **WH:** R16 สะสมตั้งแต่อัพ R01. `loadR16` แทนที่ maps ทั้งก้อน + `reEvaluateAuditItems` (audit/pass ที่ยังไม่มี auditor)
...
| **คลัง** (warehouse) | WH: มุก, ตั๋ง, แล็ค, พี |
...
**Warehouse role (`'warehouse'`)** — 4 staff (มุก, ตั๋ง, แล็ค, พี). นับสินค้าครั้งแรก **และ** สแกนรีเช็ค audit ได้ แต่ **ยืนยันไม่ได้** (`canVerify` ไม่รวม warehouse — เปิด Audit Verify popup ไม่ได้, ไอคอน 🔍 บน PDA ไม่ขึ้น).
...
- มี 2 ปุ่มบน Desktop (`updateScanInputMode`, gate `currentRole==='supervisor' && window.innerWidth>600`): **✔ Confirm** (นับ → `evaluatePendingScans`) และ **✓ ยืนยันรีเช็คทั้งหมด** (`pharmacistAuditBar`, label สลับผ่าน `#pharmacistAuditBtnLabel`)
- ปุ่มยืนยันรีเช็คทั้งหมดเรียก `confirmRecheckBtn()` → `confirmAllRecheckSupervisor()`: วนทุก audit item ที่มี `recheckQty !== undefined && !auditor` → เทียบ `recheckQty` vs `systemQty` → `pass` / `stock_adjustment`, ตั้ง `auditor = มายด์` → sync + `saveAuditLogToFirestore`
- **Confirm นับครั้งแรกทีละพนักงาน** (`#supervisorCountBtns` เหนือ `#confirmClearRow`): `renderSupervisorCountButtons()` group `scanning` items ตาม `scannedBy` → ปุ่มน้ำเงินต่อพนักงาน "✔ Confirm: <ชื่อ> (<จำนวน>)" → `confirmCountByStaff(name)` (เช็ค R16 loaded + date mismatch เหมือน `validateAndProcess` → `evaluatePendingScans(name)`). `evaluatePendingScans(staffFilter=null)` รับ param filter: เรียกไม่ใส่ = ทำทุก scanning เหมือนเดิม, ใส่ชื่อ = เฉพาะ `scannedBy === name`. ทำให้มายด์ confirm ของคนที่นับโซนเสร็จได้เลยโดยไม่ต้องรอครบ (แต่ละโซนนับเสร็จไม่พร้อมกัน + คนเสร็จก่อนไปรีเช็คต่อได้) — ของคนอื่นยังค้าง `scanning` ไม่ถูกแตะ
- **ยืนยันรีเช็คทีละพนักงาน** (`#supervisorStaffBtns` ในกล่อง `pharmacistAuditBar`): `renderSupervisorRecheckButtons()` group audit ที่รอยืนยันตาม `recheckBy` → สร้างปุ่มเขียวต่อพนักงาน "✓ ยืนยันรีเช็ค: <ชื่อ> (<จำนวน>)" → `confirmRecheckByStaff(name)` (logic เดียวกับ `confirmAllRecheckSupervisor` แต่ filter `recheckBy === name`). ทำให้มายด์กดยืนยันของคนที่สแกนเสร็จได้เลยโดยไม่ต้องรอครบทุกคน (พนักงาน PDA สแกนเสร็จไม่พร้อมกัน). render ใน `updatePharmacistAuditConfirmBtn()` → refresh สดทุก snapshot/`updateStats`. item ที่ไม่มี `recheckBy` (data เก่า) รวมเป็นปุ่ม "(ไม่ระบุ)"
- ปุ่มต่อพนักงานทั้งสองชุด (Confirm นับ + ยืนยันรีเช็ค) render ใน `updatePharmacistAuditConfirmBtn()` ด้วยกัน → refresh สดตอน PDA sync
- count บนปุ่ม (`#pharmacistAuditCount`) นับ audit item ที่รีเช็คแล้วรอยืนยัน (refresh ใน `updateStats`)
- **Layout ปุ่มยืนยัน (WH supervisor Desktop เท่านั้น) — `body.wh-sup`:** `updateScanInputMode()` toggle class `wh-sup` บน body เมื่อ `isSvDesktop && currentBranch==='WH'`. CSS `body.wh-sup` จัด: `#btnConfirm` (label "✓ Confirm ทั้งหมด" set ผ่าน JS) + `#pharmacistAuditBtn` (สีเขียว, "✓ ยืนยันรีเช็คทั้งหมด") เป็นปุ่มหลัก **width:auto + white-space:nowrap** (ขยายแนวนอนตามข้อความ ไม่ตัด, `min-width:150px`, `min-height:56px`); `#supervisorCountBtns` (ฟ้า) + `#supervisorStaffBtns` (เขียว) เป็นปุ่มรายคน **40×40** (ชื่อ+จำนวน 2 บรรทัด, `order:1` วางขวาปุ่มหลัก); `#btnHideList` (ซ่อนรายการเดิมในแถว) ซ่อน → ใช้ `#btnHideListWh` (แยกล่างสุด) แทน. ใช้ `!important` เพื่อ override inline `flex:1`/`width:100%`/`background`. **role อื่น (สาขา assistant/pharmacist, WH PDA) ไม่มี class → layout เดิม** (ปุ่มเต็มแถว, ปุ่มรายคนซ่อนเพราะ render เฉพาะ supervisor)
  - **ปุ่มรายคนแสดงครบทุกคน + disable ถ้าไม่มีงาน:** `renderSupervisorCountButtons` / `renderSupervisorRecheckButtons` วน `EMPLOYEE_PROFILES[currentBranch].warehouse` (+ ชื่ออื่นที่มีงานแต่ไม่อยู่ในลิสต์) → ปุ่มต่อคนเสมอ; ถ้า `count===0` ใส่ `disabled` + `opacity:0.38` + `cursor:not-allowed` (กดไม่ได้). count นับจาก scanData (Confirm = `scanning`/`scannedBy`; รีเช็ค = `audit`+`recheckQty`+`!auditor`/`recheckBy`)
...
- Filter: zone dropdown (`A`–`O`) + staff dropdown (มุก/ตั๋ง/แล็ค/พี) + text search
...
>
> ⚠️ **`startNewCount()` ต้องลบ master doc `${branch}_r01` ด้วย** — R01 ถูกเก็บ 2 ที่: session doc (`stock_sessions/${branch}`, sync ผ่าน `syncToFirestore`) + **master doc (`${branch}_r01`, เขียนโดย `syncMasterToFirestore`)**. ถ้าลบแค่ session doc → ตอน reload `restoreMasterFromFirestore()` อ่าน `${branch}_r01` doc เก่า → กู้ `r01Data` + `r01UploadedAt` กลับมา (badge เด้ง "Ready", timestamp อัพเก่าโผล่). `startNewCount` จึง `await ...doc(getSessionId()+'_r01').delete()` หลัง `syncToFirestore(true)` (ข้ามใน `_adminMode`). **ไม่ลบ `_r05`** — barcode mapping คงไว้ข้ามรอบนับ
>
...
**R05 real-time sync (per-branch, `startR05Listener`/`stopR05Listener`):** R05 ยังเป็น per-branch (`${branch}_r05`) แต่มี `onSnapshot` listener แล้ว (mirror PM) → **อัพ R05 เครื่องนึง → ทุกเครื่องในสาขาเดียวกันเห็น barcode ใหม่ทันที** (เดิมต้องอัพทุกเครื่อง/clear เพราะ `restoreMasterFromFirestore` early-return ถ้ามี R05 local). Listener apply cloud R05 → `state.r05Data` → `rebuildMaps()` (preserve scanData — ไม่กระทบของที่นับ). กัน **stale-push**: R05 เขียน cloud **เฉพาะตอนอัพ R05 จริง** (`loadR05` → `syncMasterToFirestore(true)`); login + `loadR01` ส่ง `syncMasterToFirestore(false)` (R01 only, ไม่ทับ cloud R05). `_lastAppliedR05Json` กัน own-write rebuild ซ้ำ + skip snapshot ที่ไม่เปลี่ยน. ⚠️ **ยังไม่ใช่ shared ข้ามสาขา** (แต่ละสาขายัง R05 แยก) — plan global_r05 ถูกพักไว้
...
**Session doc fields (localStorage + Firestore `session_data_json`):** `r01UploadedAt`, `r01BaselineAt` (สาขายา — epoch อัพ R01 สำหรับรีเซ็ต Audit ข้ามเครื่อง), `countResetAt`, `r16SalesMap`, `r16InboundMap`, `r16Loaded`, `scanData`, `scanListMap`, `unknownScans`, ฯลฯ
...
### R01 daily reset — `resetUnverifiedAuditForNewR01()` / `_applyR01BaselineUpdate()`

**เฉพาะสาขายา (`_isPharmacyBranch()`).** เรียกจาก `loadR01` (เครื่องที่อัพ) และ `_applyR01BaselineUpdate` (เครื่องอื่นผ่าน `r01BaselineAt` ใน cloud).

**รีเซ็ต:** ทุก `scanData` entry ที่ `status==='audit' && !sd.auditor` → `pending` (fields เหมือน `removeScanItem` แต่คง `location`). ลบจาก `scanListMap` + `_avMap`. ไม่แตะ `pass`/`stock_adjustment` ที่มี `auditor`.

**หลังรีเซ็ต:** `reEvaluateAuditItems({silent:true})` — ปรับ `pass` ที่ยังไม่ยืนยันตาม R16+baseline ปัจจุบัน (ไม่ toast).

**Cross-device triggers:** `startScanSessionListener` (ก่อน `_applyCloudScanData`), `syncToFirestore` (ก่อน merge), `pullFromCloud`, `restoreFromFirestore` early-return path — ทั้งหมดเช็ค `s.r01BaselineAt > _r01BaselineAt`.

---

### R16 Re-evaluation — `reEvaluateAuditItems(opts={})`

เรียกอัตโนมัติทุกครั้งที่อัพ R16 ใหม่ (ถ้า matched > 0) เพื่อแก้สถานะที่อาจผิดจาก R16 ผิดไฟล์. เรียกจาก `loadR01` สาขายาด้วย (`silent:true`). `opts.silent` กด toast "R16 ใหม่: ปรับสถานะ N รายการ"
...
**สาขายา:** R01 อัพได้ทุกวัน (baseline หลัง restock). R16 อัพรายวันได้ — export ช่วงหลังอัพ R01 ถึง Confirm. **WH:** R01 ครั้งเดียวต่อรอบ; R16 สะสมตั้งแต่อัพ R01. สูตร: `effectiveCnt = countedQty + soldQty_before_scan + r16103 - inbound` เทียบ `systemQty`
...
- **R01.102**: Col E (index 4)=SKU, F (5)=ProductName, G (6)=SystemQty; ข้ามเฉพาะแถวที่ qty **ว่าง/ไม่ใช่ตัวเลข** (junk). **qty≤0** เก็บค่าจริง (DEL ที่ติดลบ/0 นับได้). Re-upload → `rebuildMaps()` อัปเดต `systemQty` ทุก SKU; `scanData` ทั่วไปไม่รีเซ็ต — **ยกเว้นสาขายา:** `resetUnverifiedAuditForNewR01()` รีเซ็ตเฉพาะ `audit` ที่ยังไม่มี `auditor`. ล้างทั้งก้อน: `startNewCount()` / `resetStaleScanningItems()` (login, scanning ข้ามวัน)
  - **Re-upload baseline:** Confirm + Audit Verify อ้างอิง `systemQty` ปัจจุบันเสมอ. **สาขายา:** อัพ R01 ทุกวันได้ตั้งใจ (restock) — Audit ค้างรีเซ็ตให้ผู้ช่วยนับใหม่. **WH:** อย่าอัพ R01 กลางรอบ recheck. R01 persist ใน `${branch}_r01` master doc. SKU หายจากไฟล์ใหม่ → รีเช็คไม่ได้ (`si` undefined)
...
- **R16.104**: Col C (2) กำหนดประเภทเอกสาร — กรองเฉพาะ 5 ประเภทนี้ ที่เหลือข้ามทั้งหมด:
  ...
  | `OTFB`, `ORTS` | รับเข้าคลัง (Inbound) | **หักออก** (`r16InboundMap`) — KKL/SSS: OTFB ทุกแถว. **SRC:** OTFB Col A=`0`/ว่าง → **ข้าม** (คลังส่งไปสาขาอื่น ไม่กระทบ SRC); Col A=`1` → **บวกกลับ** (สาขาส่งออก) |
  | `OTFI` | **branch-aware ด้วย Col A** | **สาขา:** Col A=`1` (สาขา→คลัง) → **บวกกลับ**; Col A=`0`/ว่าง → **หักออก**. **WH:** OTFI **หักออก** เสมอ |
...
**DEL ที่ระบบติดลบ/0 ก็นับได้** — loadR01 เก็บ qty≤0 ไว้ด้วยค่าจริง (ไม่ตัดทิ้ง) → DEL ที่ systemQty ≤0 แต่มีของจริงบนชั้น จะโผล่มาให้นับ (เดิมโดน filter qty≤0 ตัดหาย). systemQty อาจติดลบ → นับของจริง (เช่น 2) เทียบกับติดลบ (เช่น −3) = Audit, Diff สะท้อนการปรับสต็อกจริง (+5). ⚠️ filter `'pending'` ใน popup ไม่รวม DEL → ดู DEL ที่ยังไม่นับผ่านปุ่ม **🗑️ DEL**
...
⚠️ **WH PDA (`noEditPda`) — `scanning` ที่ `systemQty > 100` เปิด inline input แล้ว** (ของจำนวนเยอะพิมพ์ได้ parity กับสาขา PDA); `scanning` ≤100 + `audit` (recheck) ยังเป็น `<span>` สแกนอย่างเดียว. เงื่อนไข scanning ใหม่ (renderScanList + patchScanRow ตรงกัน): **`sysQty>100 || (whStaffEdit && !noEditPda)`** → WH PDA ได้ input เมื่อ >100, WH Desktop ได้เสมอ, สาขาได้เมื่อ >100. warehouse นับ ≤100 ด้วยการสแกน (ยิง barcode กล่อง × `unitMultiplier` / พิมพ์ `barcode,qty`, `parseScanLine` รองรับ); แก้ผิด → ปุ่ม **✕** (`removeScanItem`/`resetRecheckItem`). **audit (recheck) ยัง gate `&& !noEditPda`** → WH PDA สแกนรีเช็คอย่างเดียว

inline `<input>` แยกตามกรณี — **`scanning`:** WH PDA (เมื่อ `systemQty>100`) + WH Desktop (เสมอ); **`audit` (recheck):** **WH Desktop เท่านั้น** (`innerWidth>600`):
...
> ⚠️ **กฎสำหรับ Claude — Bump APK เฉพาะเมื่อแก้ native Android เท่านั้น:**
>
...
> - `CLAUDE.md`, `WORKFLOW.md`, `README.md`, `APK.md` — docs ล้วน ไม่กระทบ runtime
