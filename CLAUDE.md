...
- **warehouse** — แสดงของตัวเอง (`sd.scannedBy === currentUser`) **แต่** audit items แสดงของ**ทุกคน** (worklist รีเช็ค, ข้าม filter `scannedBy`); แถว audit โชว์ `sd.recheckQty`. **บน PDA (`window.innerWidth<=600`): แยก 2 tab** ในแถว header (id `whResultTabs`, แทน label "RESULT") — **"รอยืนยัน"** (`_whResultTab==='result'`, default) โชว์ `scanning` (นับครั้งแรก รอ supervisor confirm) · **"รีเช็ค"** (`_whResultTab==='recheck'`) โชว์ `audit` (worklist). `rebuildScanListMap` กรอง `sd.status !== (_whResultTab==='recheck'?'audit':'scanning')` → แต่ละ tab โชว์ status เดียว (ซ่อน `pass`/`stock_adjustment`/`audit_check` อัตโนมัติ — ไม่งั้น pass ตัวเอง filter แค่ `scannedBy` จะปนเต็ม). `setWhResultTab(tab)` สลับ + rebuild; `updateWhResultTabs()` อัปเดต active + จำนวนบน tab (เรียกใน `updateStats` + `updateScanInputMode`); toggle โชว์ tab ใน `updateScanInputMode` (`_whPda && currentRole==='warehouse'`). Desktop warehouse ไม่มี tab/ไม่กรอง (เห็น pass ด้วย)
