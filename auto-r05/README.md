...
|---|---|---|---|
| 1 | Task `BOT05106` (ProMaxx GUI automation · โฟลเดอร์ `BOTR05106`) | export R05.106 → `Desktop\run-upload-stock\R05.106.CSV` แล้วอัปเข้า Supabase ของอีกโปรเจกต์ | ไฟล์เขียนเสร็จ ~07:55 (วัดจริง 9 ก.ย. 2026 — ดู §ขั้น 0 ทุกครั้งก่อนตั้ง) |
| 2 | **`auto-r05` (โฟลเดอร์นี้) · Task `AutoR05Import`** | **อ่านไฟล์นั้น → เขียน `global_r05` บน Firestore** | 09:00 (คนตั้งเลือกเอง ขอแค่หลังข้อ 1) |
| 3 | `auto-r01` · Task `AutoR01Import` | อ่าน `Allstock.CSV` → เขียน `{branch}_r01` ทั้ง 4 สาขา | 08:10 |
...
เห็น exit 4 ติดกันหลายวัน แปลว่าต้องไปแก้ที่ Task `BOT05106` ไม่ใช่ที่นี่
...
### doc สถานะแยกต่างหาก: `stock_sessions/global_r05_status` (ก.ย. 2026)

เขียนทุกรอบที่ **ตรวจสำเร็จ** — ทั้งรอบที่เขียนข้อมูลจริงและรอบที่ "เนื้อหาเหมือนเดิมจึงไม่เขียน"

| field | ค่า |
|---|---|
| `checked_at` | เวลาที่บอทตรวจเสร็จ (หน้าเว็บเอาไปโชว์ "ตรวจล่าสุด" บนการ์ด R05.106) |
| `result` | `written` หรือ `unchanged` |
| `row_count` · `source_file` · `source_mtime` · `machine` | ไว้ไล่ย้อนว่ารอบนั้นอ่านไฟล์ไหนจากเครื่องไหน |

⚠️ **ตกด่านเมื่อไรห้ามเขียน** (ไฟล์ไม่ใช่ของวันนี้ · หัวคอลัมน์เพี้ยน · แถวหด · ราคาหาย) — การ์ดต้องเตือนได้จริง
ไม่งั้นจะขึ้นเขียวทั้งที่บอทหยุดทำงาน ซึ่งแย่กว่าไม่มีสัญญาณเลย · เขียนไม่สำเร็จถือว่าไม่ร้ายแรง (log warn แล้วไปต่อ)

**ทำไมต้องแยก doc:** เพราะการอัปผ่านหน้าเว็บเป็น `set()` ทั้ง document ⇒ field ส่วนเกินใน `global_r05` จะถูกลบทิ้ง
เหตุผลเดียวกับข้อห้ามด้านล่าง — ที่นี่จึงเป็น doc ของบอทล้วน ๆ ไม่มีใครเขียนทับ

⛔ **ห้ามเติม field ของบอทเองลงไปใน `global_r05`** — ผลของบอทต้องแยกไม่ออกจากคนอัปผ่านหน้าเว็บ
...
**วิธีที่ง่ายที่สุด — ดูจากการ์ด R05.106 ในหน้าเว็บ** (ก.ย. 2026)
เข้า Admin Mode แล้วการ์ดจะขึ้น `อัปโหลดล่าสุด: HH:MM น. DD/MM/YYYY` เหมือนการ์ด R01.102
เวลานี้มาจาก field `updated_at` ของ `global_r05` โดยตรง ⇒ **เป็นเวลาที่บอทเขียนจริง ไม่ใช่เวลาที่เครื่องนี้โหลด**
⚠️ วันที่ไม่ขยับไม่ได้แปลว่าบอทพัง — ถ้าตารางบาร์โค้ดไม่เปลี่ยน บอทจะไม่เขียน (ด่านที่ 7) เวลาจึงค้างที่รอบก่อนโดยตั้งใจ
ให้ไปดู `auto_r05.log` ว่ารอบล่าสุดจบด้วย "เนื้อหาเหมือนเดิม" (ปกติ) หรือ exit 4 (มีปัญหา)
