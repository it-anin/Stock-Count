...
- **คอลัมน์ P = หมวดที่ไม่ใช่สินค้าคงคลัง → ไม่ต้องนับ (ส.ค. 2026)** — `_isNonCountR01Category()` ติดธง `nc:1` ตอน `loadR01`
  - **ธง `nc` มี 2 ชนิด (ก.ย. 2026):**

    | ธง | หมวด | พฤติกรรม |
    |---|---|---|
    | `nc:1` | ขึ้นต้น `11.` (`R01_NON_COUNT_PREFIXES`) | ตัดเด็ดขาด ชนะทั้งยอดและชั้น A/B/C/REVIEW |
    | `nc:2` | มีคำว่า `DELETE` (`R01_STOCK_ONLY_KEYWORDS`) | นับเฉพาะ `G ≠ 0` — **ชั้น A/B/C/REVIEW ดึงของยอด 0 เข้าไม่ได้** |
    | ไม่มีธง | หมวดปกติ | `G ≠ 0` **หรือ** ชั้น A/B/C/REVIEW |

  - ⛔ **`DELETE` อยู่ใน `R01_STOCK_ONLY_KEYWORDS` เท่านั้น ห้ามย้ายกลับไป `R01_NON_COUNT_KEYWORDS`**
    745 รายการยังมียอดคงเหลือ (ติดลบ 29) และมีบาร์โค้ดครบทุกตัว = ของบนชั้นที่ต้องเดินไปนับ · ส่วนที่ยอด 0 (~6,100) ต้องไม่โผล่เป็นงานแม้จัดชั้น A/B/C/REVIEW ไว้
    `R01_NON_COUNT_KEYWORDS` ต้องคงอยู่เป็น array ว่าง — `tools/check-r01-parity.js` ดึงบรรทัดนี้ด้วย regex ลบแล้ว exit 2
  - ⚠️ อ่านธงแบบ truthy ตอน sticky แล้วเก็บค่าที่เข้มที่สุด (`1` ชนะ `2`) — doc เก่าที่มีแต่ `nc:1` ต้องให้ผลเท่าเดิมเป๊ะ
  - ⚠️ **กติกาอยู่ 3 ที่ ต้องแก้พร้อมกัน:** `index.html` · `auto-r01/auto_r01_import.py` · `tools/list-r01-categories.js` แล้วรัน `node tools/check-r01-parity.js "<Allstock.CSV>"` (ต้อง exit 0 · checker ดึงทั้ง 2 ชนิดมาเทียบ)
  - ⚠️ ธง `nc` ตรึงลง cloud ตอน parse — แก้กติกาแล้วต้อง `python auto_r01_import.py --resync-nc --yes` ถ้าอยากให้มีผลกับรอบนับที่ทำอยู่ (เขียนเฉพาะ `data_json` ไม่แตะ version/baseline/r16 · ต้องรันก่อน deploy)
...
- **badge แดง `DEL` ขึ้นทุกตัวที่ `isDel`** (`renderPopupTable`) แต่ **ปุ่ม filter 🗑️ DEL กรองด้วย `_countableSkus` ด้วย** (ส.ค. 2026)
  เจตนา: ปุ่มนี้คือ "งานที่ต้องเดินไปหา" ไม่ใช่รายงานสินค้านอกแคตตาล็อกทั้งหมด → DEL ที่ของหมดแล้วหรืออยู่ในหมวด `11.` ไม่โผล่
  ⚠️ ก.ย. 2026: DEL ที่มาจากหมวด `DELETE` **โผล่ในปุ่มนี้แล้ว**ถ้ายังมียอด (776 รายการทั้งระบบ) — เป็นงานที่ตั้งใจให้เห็น ไม่ใช่บั๊ก
