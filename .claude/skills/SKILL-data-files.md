...
- **คอลัมน์ P = หมวดที่ไม่ใช่สินค้าคงคลัง → ไม่ต้องนับ (ส.ค. 2026)** — `_isNonCountR01Category()` ติดธง `nc:1` ตอน `loadR01`
  - ตัด: ขึ้นต้นด้วย **`11.`** (`R01_NON_COUNT_PREFIXES`) เท่านั้น — เพิ่มหมวดใหม่เติม 2 array นี้ที่เดียว
  - ⛔ **`R01_NON_COUNT_KEYWORDS` เป็น array ว่างโดยเจตนา (ก.ย. 2026) ห้ามเติม `DELETE` กลับ และห้ามลบตัวแปรทิ้ง**
    ของจริง 776 รายการหมวด DELETE ยังมียอดคงเหลือ (SRC 327 · KKL 220 · SSS 224 · WH 5 · ติดลบ 29) และมีบาร์โค้ดครบทุกตัว = ของบนชั้นที่ต้องเดินไปนับ · ตัวที่ยอด 0 ถูกกติกา `G ≠ 0` ตัดออกเองอยู่แล้ว
    ตัวแปรต้องคงอยู่เพราะ `tools/check-r01-parity.js` ดึงบรรทัดนี้ด้วย regex — ลบแล้ว exit 2
  - ⚠️ **กติกาอยู่ 3 ที่ ต้องแก้พร้อมกัน:** `index.html` · `auto-r01/auto_r01_import.py` · `tools/list-r01-categories.js` แล้วรัน `node tools/check-r01-parity.js "<Allstock.CSV>"` (ต้อง exit 0)
